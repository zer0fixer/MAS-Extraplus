...
default monika_costumes_ = [(_("Blanket"), blanket_monika), (_("Android"), android_chibi), (_("Casual"), casual_chibi)]
default natsuki_costumes_ = [(_("Blanket"), blanket_nat)]
default sayori_costumes_ = [(_("Blanket"), blanket_sayo)]
default yuri_costumes_ = [(_("Blanket"), blanket_yuri)]
...
init 5 python:
    ...
    def make_bday_oki_doki():
        ...
        try:
            ...
            if not renpy.android:
                ...
                with open(os.path.join('/storage/emulated/0/MAS', 'characters', 'oki doki'), 'w') as f:
                    pass
    ...
    def get_formatted_time_since_install():
        ...
        if not (persistent.sessions
            ...
            return renpy.substitute(_("a wonderful time"))
        ...
        try:
            ...
            if total_days < 1:
                return renpy.substitute(_("less than a day, but every second has been incredible!"))
            ...
            parts = []
            if years > 0:
                parts.append("{0} {1}".format(years, renpy.substitute(_("year")) if years == 1 else renpy.substitute(_("years"))))
            if months > 0:
                parts.append("{0} {1}".format(months, renpy.substitute(_("month")) if months == 1 else renpy.substitute(_("months"))))
            if days > 0:
                parts.append("{0} {1}".format(days, renpy.substitute(_("day")) if days == 1 else renpy.substitute(_("days"))))
            ...
            return renpy.substitute(_("an unforgettable time"))
    ...
    def extra_get_mas_stats():
        ...
        if not persistent.sessions:
            return {
                ...
                _("Visits to The Spaceroom"): "0",
                _("Our Time Together"): _("N/A"),
                _("Average Time per Visit"): _("N/A")
            }
        ...
        # 修复日期显示问题 - 分别提取年、月、日然后用中文拼接
        if first_session:
            year = first_session.year
            month = first_session.month
            day = first_session.day
            stats[_("The Day We Met <3")] = "{0}年{1}月{2}日".format(year, month, day)
        else:
            stats[_("The Day We Met <3")] = _("Unknown")

        # --- Format data for display with friendly names ---
        #stats[_("The Day We Met <3")] = first_session.strftime(renpy.substitute(_("%B %d, %Y"))) if first_session else _("Unknown")
        stats[_("Visits to The Spaceroom")] = str(total_sessions)
        h, rem = divmod(total_playtime.total_seconds(), 3600)
        m, s = divmod(rem, 60)
        stats[_("Our Time Together")] = renpy.substitute(_("{:02.0f}h {:02.0f}m")).format(h, m)
        if total_sessions > 0:
            ...
            stats[_("Average Time per Visit")] = renpy.substitute(_("{:02.0f}h {:02.0f}m")).format(h_avg, m_avg)
        else:
            stats[_("Average Time per Visit")] = _("N/A")
    ...
    class extra_gift:
        ...
        def __call__(self):
            if not renpy.android:
                ...
                file_path = os.path.join('/storage/emulated/0/MAS', 'characters', self.gift)
            with open(file_path, 'a') as f:
                messages = [
                    renpy.substitute(_("Oh! You got me a {}! Thank you!")).format(self.name),
                    renpy.substitute(_("A {}? For me? You're so sweet!")).format(self.name),
                    renpy.substitute(_("Wow! Is this a {}? I love it!")).format(self.name),
                    renpy.substitute(_("You shouldn't have! But I'm so happy you gave me a {}!")).format(self.name),
                    renpy.substitute(_("Eee! A {}! You're the best!")).format(self.name)
                ]
